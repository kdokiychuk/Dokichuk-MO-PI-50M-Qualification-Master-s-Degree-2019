...
@section('contentPage')
    <h1>Кількість товарів = {{$allTovars}} <br>Натисність для редагування запису <a href="{{ url('operator/tovar/create') }}" class="btn btn-success">Створити TOVAR</a></h1>
    <h2>
        <form class="form-horizontal" action="{{  url('operator/tovar/view') }}" method="post" style="margin-bottom: 5px">
            {{csrf_field()}}
        ...
        <a href="{{  url('operator/tovar/view') }}" style="margin-bottom: 50px; display: block" class="btn btn-warning">Переглянути всі Товари</a>
    </h2>
    <table class="table table-striped tableView">
        ...
        @foreach ($masRoleWorker as $role)
            <tr>
                ...
                <td><?= $role->id_type_tovar ?><br><?= $role->id_type_oplata_tovar ?></td>
                <td><?= $role->price ?></td>
                <td>
                    <form action="{{url('operator/tovar/view/'.$role->id_tovar) }}" method="post">
                        {{csrf_field()}}
                        {{method_field("DELETE")}}
